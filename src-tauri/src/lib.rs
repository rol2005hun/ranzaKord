...
fn set_discord_presence(
    ...
    if let Some(client) = client_guard.as_mut() {
        ...
        activity = activity.assets(activity::Assets::new().large_image(&image).large_text("ranzaKord"));
        ...
        if let Err(e) = client.set_activity(activity.clone()) {
            eprintln!("[Discord RPC] Failed to set activity: {:?}. Attempting reconnect...", e);
            let _ = client.close();
            if let Err(e2) = client.connect() {
                eprintln!("[Discord RPC] Failed to connect: {:?}", e2);
            } else if let Err(e3) = client.set_activity(activity) {
                eprintln!("[Discord RPC] Failed to set activity after connect: {:?}", e3);
            }
        }
    }
    Ok(())
